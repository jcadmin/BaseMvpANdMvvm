 ...
 * sending capture request by a {@link Request} consisting of target output
 ...
 * specified in {@link Request}s can be mapped to the target Surface by checking
...
public class Camera2RequestProcessor implements RequestProcessor {
    ...
    private boolean areRequestsValid(@NonNull List<Request> requests) {
    ...
    private boolean isRequestValid(@NonNull Request request) {
     ...
     * {@link Callback}. Due to the CaptureSession design, each request has
     ...
     * {@link Callback} to avoid duplicate invoking.
     */
    private class Camera2CallbackWrapper extends CameraCaptureSession.CaptureCallback {
        ...
        Camera2CallbackWrapper(@NonNull Request captureRequest,
                @NonNull Callback callback, boolean invokeSequenceCallback) {
            mCallback = callback;
